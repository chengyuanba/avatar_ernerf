...
from ernerf.encoders.encoding import get_encoder
from ernerf.networks.renderer import NeRFRenderer
...
class AudioNet(nn.Module):
    def __init__(self, dim_in=29, dim_aud=64, win_size=16):
        ...
        self.encoder_conv = nn.Sequential(  # n x 29 x 16
            nn.Conv1d(dim_in, 32, kernel_size=3, stride=2,
...
class NeRFNetwork(NeRFRenderer):
    def __init__(self, opt,
        ...
        if audio_in_dim != -1:
            ...
            assert ("data2vec-audio" in self.opt.asr_model) or \
                ("wav2vec2" in self.opt.asr_model) or \
                ("hubert" in self.opt.asr_model) or \
                ("whisper" in self.opt.asr_model), \
                "Only support ['data2vec-audio', 'hubert', 'wav2vec2', 'whisper']."

            if ("wav2vec2-base" in self.opt.asr_model) or \
                    ("hubert-base" in self.opt.asr_model):
                self.audio_in_dim = 768
            elif ("wav2vec2-large" in self.opt.asr_model) or \
                    ("hubert-large" in self.opt.asr_model):
                self.audio_in_dim = 1024
            elif "whisper-large" in self.opt.asr_model:
                self.audio_in_dim = 1280

        if self.emb:
            self.embedding = nn.Embedding(self.audio_in_dim, self.audio_in_dim)

        # audio network
        self.audio_dim = audio_out_dim
        ...
        self.encoder_yz, self.in_dim_yz = get_encoder("hashgrid", input_dim=2, num_levels=self.num_levels,
                                                      level_dim=self.level_dim, base_resolution=64, log2_hashmap_size=14, desired_resolution=512 * self.bound)
        ...
        self.sigma_net = MLP(self.in_dim + self.audio_dim + self.eye_dim,
                             1 + self.geo_feat_dim, self.hidden_dim, self.num_layers)
        # color network
        self.num_layers_color = 2
        self.hidden_dim_color = 64
        self.encoder_dir, self.in_dim_dir = get_encoder("spherical_harmonics")
        self.color_net = MLP(self.in_dim_dir + self.geo_feat_dim +
                             self.individual_dim, 3, self.hidden_dim_color, self.num_layers_color)
        ...
        if self.torso:
            ...
            self.torso_deform_net = MLP(
                self.torso_deform_in_dim + self.anchor_in_dim + self.individual_dim_torso, 2, 32, 3)

            # torso color network
            self.torso_encoder, self.torso_in_dim = get_encoder(
                "tiledgrid", input_dim=2, num_levels=16, level_dim=2, base_resolution=16, log2_hashmap_size=16, desired_resolution=2048)
            self.torso_net = MLP(self.torso_in_dim + self.torso_deform_in_dim +
                                 self.anchor_in_dim + self.individual_dim_torso, 4, 32, 3)
    ...
    def forward_torso(self, x, poses, c=None):
        ...
        # deformation-based
        wrapped_anchor = self.anchor_points[None, ...] @ poses.permute(
            0, 2, 1).inverse()
        wrapped_anchor = (wrapped_anchor[:, :, :2] / wrapped_anchor[:,
                          :, 3, None] / wrapped_anchor[:, :, 2, None]).view(1, -1)
        ...
        if c is not None:
            h = torch.cat([enc_x, enc_anchor.repeat(
                x.shape[0], 1), c.repeat(x.shape[0], 1)], dim=-1)
        else:
    ...
    def encode_audio(self, a):
        ...
        if self.emb:
            a = self.embedding(a).transpose(-1, -
                                            2).contiguous()  # [1/8, 29, 16]
    ...
    def get_params(self, lr, lr_net, wd=0):
        ...
        if self.torso:
            params = [
                ...
                {"params": self.torso_deform_encoder.parameters(), "lr": lr,
                 "weight_decay": wd},
                {"params": self.torso_net.parameters(), "lr": lr_net,
                 "weight_decay": wd},
                {"params": self.torso_deform_net.parameters(), "lr": lr_net,
                 "weight_decay": wd},
            ...
            if self.individual_dim_torso > 0:
                params.append(
                    {"params": self.individual_codes_torso, "lr": lr_net, "weight_decay": wd})

            return params

        params = [
            {"params": self.audio_net.parameters(), "lr": lr_net,
             "weight_decay": wd},
        ...
        if self.att > 0:
            params.append({"params": self.audio_att_net.parameters(),
                          "lr": lr_net * 5, "weight_decay": 0.0001})
        if self.emb:
            params.append({"params": self.embedding.parameters(), "lr": lr})
        if self.individual_dim > 0:
            params.append({"params": self.individual_codes,
                          "lr": lr_net, "weight_decay": wd})
        if self.train_camera:
            params.append({"params": self.camera_dT,
                          "lr": 1e-5, "weight_decay": 0})
            params.append({"params": self.camera_dR,
                          "lr": 1e-5, "weight_decay": 0})

        params.append({"params": self.aud_ch_att_net.parameters(),
                      "lr": lr_net, "weight_decay": wd})
        params.append({"params": self.unc_net.parameters(),
                      "lr": lr_net, "weight_decay": wd})
        params.append({"params": self.eye_att_net.parameters(),
                      "lr": lr_net, "weight_decay": wd})
