...
from ernerf import raymarching
from ernerf.networks.utils import (custom_meshgrid, euler_angles_to_matrix,
                                   get_audio_features)
...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        aabb_train = torch.FloatTensor(
            [-opt.bound, -opt.bound/2, -opt.bound, opt.bound, opt.bound/2, opt.bound])
        ...
        if self.individual_dim > 0:
            self.individual_codes = nn.Parameter(torch.randn(
                self.individual_num, self.individual_dim) * 0.1)

        if self.torso:
            self.individual_dim_torso = opt.ind_dim_torso
            if self.individual_dim_torso > 0:
                self.individual_codes_torso = nn.Parameter(torch.randn(
                    self.individual_num, self.individual_dim_torso) * 0.1)

        # optimize camera pose
        self.train_camera = self.opt.train_camera
        if self.train_camera:
            self.camera_dR = nn.Parameter(torch.zeros(
                self.individual_num, 3))  # euler angle
            self.camera_dT = nn.Parameter(torch.zeros(
                self.individual_num, 3))  # xyz offset
    ...
    def run_cuda(self, rays_o, rays_d, auds, bg_coords, poses, eye=None, index=0, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        if self.train_camera and (self.training or self.test_train):
            ...
            dR = euler_angles_to_matrix(
                self.camera_dR[index] / 180 * np.pi + 1e-8).squeeze(0)  # [1, 3] --> [3, 3]
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(
                ...
                self.mean_count, perturb, 128, force_all_rays, dt_gamma, max_steps
            )
            ...
            weights_sum, amb_aud_sum, amb_eye_sum, uncertainty_sum, depth, image = raymarching.composite_rays_train_triplane(
                sigmas, rgbs, amb_aud.abs().sum(-1), amb_eye.abs().sum(-1), uncertainty, deltas, rays)
            ...
            while step < max_steps:
                ...
                raymarching.composite_rays_triplane(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambients_aud,
                                                    ambients_eye, uncertainties, weights_sum, depth, image, amb_aud_sum, amb_eye_sum, uncertainty_sum, T_thresh)
        ...
        torso_results = self.run_torso(
            rays_o, bg_coords, poses, index, bg_color)
    ...
    def run_torso(self, rays_o, bg_coords, poses, index=0, bg_color=None, **kwargs):
        ...
        if self.torso:
            ...
            occupancy = F.grid_sample(
                self.density_grid_torso.view(
                    1, 1, self.grid_size, self.grid_size),
                bg_coords.view(1, -1, 1, 2), align_corners=True).view(-1)
        ...
        if not self.torso:  # forbid updating head if is training torso...
            ...
            if self.exp_eye:
                eye = self.eye_area[[rand_idx]].to(
                    self.density_bitfield.device)  # [1, 1]
            else:
                eye = None
                    ...
                    for zs in Z:
                        ...
                        for cas in range(self.cascade):
                            ...
                            cas_xyzs += (torch.rand_like(cas_xyzs)
                                         * 2 - 1) * half_grid_size
                            # query density
                            sigmas = self.density(cas_xyzs, enc_a, eye)[
                                'sigma'].reshape(-1).detach().to(tmp_grid.dtype)
            ...
            self.mean_density = torch.mean(
                self.density_grid.clamp(min=0)).item()
            self.iter_density += 1
        ...
        if self.torso:
            ...
            for xs in X:
                for ys in Y:
                    ...
                    # NOTE: xy transposed!
                    indices = (coords[:, 1] *
                               self.grid_size + coords[:, 0]).long()
                    xys = 2 * coords.float() / (self.grid_size - 1) - \
                        1  # [N, 2] in [-1, 1]
                    xys = xys * (1 - half_grid_size)
                    # add noise in [-hgs, hgs]
                    xys += (torch.rand_like(xys) * 2 - 1) * half_grid_size
                    # query density
                    alphas, _, _ = self.forward_torso(
                        xys, pose, ind_code)  # [N, 1]

                    # assign
                    tmp_grid_torso[indices] = alphas.squeeze(1).float()

            # dilate
            tmp_grid_torso = tmp_grid_torso.view(
                1, 1, self.grid_size, self.grid_size)
            ...
            self.mean_density_torso = torch.mean(
                self.density_grid_torso).item()
                ...
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        cas_xyzs += (torch.rand_like(cas_xyzs)
                                     * 2 - 1) * half_grid_size
                        # query density
                        aud_norms = self.density(cas_xyzs.to(tmp_grid.dtype), enc_a, eye)[
                            'ambient_aud'].reshape(-1).detach().to(tmp_grid.dtype)
                        # assign
                ...
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        cas_xyzs += (torch.rand_like(cas_xyzs)
                                     * 2 - 1) * half_grid_size
                        # query density
                        eye_norms = self.density(cas_xyzs.to(tmp_grid.dtype), enc_a, eye)[
                            'ambient_eye'].reshape(-1).detach().to(tmp_grid.dtype)
                        # assign
