...
def get_audio_features(features, att_mode, index):
    if att_mode == 0:
        ...
        if pad_left > 0:
            ...
            auds = torch.cat([torch.zeros(
                pad_left, *auds.shape[1:], device=auds.device, dtype=auds.dtype), auds], dim=0)
        return auds
    elif att_mode == 2:
        left = index - 4
        right = index + 4
...
def convert_poses(poses):
    ...
    out = torch.empty(poses.shape[0], 6,
                      dtype=torch.float32, device=poses.device)
...
def get_bg_coords(H, W, device):
    ...
    bg_coords = torch.cat([xs.reshape(-1, 1), ys.reshape(-1, 1)],
                          dim=-1).unsqueeze(0)  # [1, H*W, 2], in [-1, 1]
    return bg_coords
...
class LMDMeter:
    ...
    def vis_landmarks(self, img, lms):
        plt.imshow(img)
        plt.plot(lms[48:68, 0], lms[48:68, 1], marker='o',
                 markersize=1, linestyle='-', lw=2)
        plt.show()
    ...
    def update(self, preds, truths):
        ...
        preds, truths = self.prepare_inputs(
            preds[0], truths[0])  # [H, W, 3] numpy array
    ...
    def write(self, writer, global_step, prefix=""):
        writer.add_scalar(os.path.join(
            prefix, f"LMD ({self.backend})"), self.measure(), global_step)
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        if not self.opt.torso:
            outputs = self.model.render(rays_o, rays_d, auds, bg_coords, poses, eye=eye, index=index, staged=False, bg_color=bg_color,
                                        perturb=True, force_all_rays=False if (self.opt.patch_size <= 1 and not self.opt.train_camera) else True, **vars(self.opt))
        else:
            outputs = self.model.render_torso(rays_o, rays_d, auds, bg_coords, poses, eye=eye, index=index, staged=False, bg_color=bg_color,
                                              perturb=True, force_all_rays=False if (self.opt.patch_size <= 1 and not self.opt.train_camera) else True, **vars(self.opt))
        ...
        if self.opt.unc_loss and not self.flip_finetune_lips:
            ...
            loss *= alpha + (1-alpha)*((1 - step_factor) +
                                       step_factor * unc_weight.detach()).clamp(0, 10)
        ...
        if self.opt.patch_size > 1 and not self.opt.finetune_lips:
            rgb = rgb.view(-1, self.opt.patch_size, self.opt.patch_size,
                           3).permute(0, 3, 1, 2).contiguous() * 2 - 1
            pred_rgb = pred_rgb.view(-1, self.opt.patch_size, self.opt.patch_size,
                                     3).permute(0, 3, 1, 2).contiguous() * 2 - 1
        ...
        if self.opt.finetune_lips:
            ...
            rgb = rgb.view(-1, xmax - xmin, ymax - ymin,
                           3).permute(0, 3, 1, 2).contiguous() * 2 - 1
            pred_rgb = pred_rgb.view(-1, xmax - xmin, ymax -
                                     ymin, 3).permute(0, 3, 1, 2).contiguous() * 2 - 1
        ...
        if self.opt.torso:
            ...
            loss_ws = - alphas * \
                torch.log2(alphas) - (1 - alphas) * torch.log2(1 - alphas)
        ...
        if self.opt.amb_eye_loss and not self.opt.torso:
            ...
            loss_cross = ((ambient_eye * ambient_aud.detach())
                          * face_mask.view(-1)).mean()
        ...
        if self.global_step % 16 == 0 and not self.flip_finetune_lips:
            ...
            xyz_delta = (torch.rand(size=xyzs.shape,
                         dtype=xyzs.dtype, device=xyzs.device) * 2 - 1) * 1e-3
            with torch.no_grad():
                sigmas_raw, rgbs_raw, ambient_aud_raw, ambient_eye_raw, unc_raw = self.model(
                    xyzs, dirs, enc_a.detach(), ind_code.detach(), eye)
            sigmas_reg, rgbs_reg, ambient_aud_reg, ambient_eye_reg, unc_reg = self.model(
            ...
            if self.opt.amb_aud_loss:
                reg_loss += self.criterion(ambient_aud_raw,
                                           ambient_aud_reg).mean()
            if self.opt.amb_eye_loss:
                reg_loss += self.criterion(ambient_eye_raw,
                                           ambient_eye_reg).mean()
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, auds, bg_coords, poses, eye=eye,
                                    index=index, staged=True, bg_color=bg_color, perturb=False, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):
        ...
        if self.opt.exp_eye and self.opt.fix_eye >= 0:
            eye = torch.FloatTensor([self.opt.fix_eye]).view(
                1, 1).to(self.device)
        else:
            eye = data['eye']  # [B, 1]
        ...
        outputs = self.model.render(rays_o, rays_d, auds, bg_coords, poses,
                                    eye=eye, index=index, staged=True,
        ...
        # (pred_rgb.cpu().numpy()[0]*255).astype("uint8")[:, :, ::-1]
        # (pred_depth.cpu().numpy()[0]*255).astype("uint8")
    ...
    def test(self, loader, save_path=None, name=None, write_image=False):
        ...
        imageio.mimwrite(os.path.join(
            save_path, f'{name}.mp4'), all_preds, fps=25, quality=8, macro_block_size=1)
        imageio.mimwrite(os.path.join(
            save_path, f'{name}_depth.mp4'), all_preds_depth, fps=25, quality=8, macro_block_size=1)

        self.log(
            f"\n==> Finished Test, save results to {os.path.join(save_path, f'{name}.mp4')}")

    # test and return the queue of images
    def test_new(self, loader):
        frame_rgb_list = []
        frame_depth_list = []
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    ...
                    image_rgb = (
                        preds[0].detach().cpu().numpy()*255).astype("uint8")
                    image_depth = (
                        preds_depth[0].detach().cpu().numpy()).astype("float32")

                    frame_rgb_list.append(image_rgb)
                    frame_depth_list.append(image_depth)

                    pbar.update(loader.batch_size)
        pbar.close()
        self.log(f"[INFO] Inference Done.")
    ...
    def test_gui(self, pose, intrinsics, W, H, auds, eye=None, index=0, bg_color=None, spp=1, downscale=1):
        ...
        if downscale != 1:
            ...
            preds = F.interpolate(preds.permute(0, 3, 1, 2), size=(
                H, W), mode='bilinear').permute(0, 2, 3, 1).contiguous()
            ...
            for data in loader:
                ...
                if self.local_rank == 0:
                    ...
                    save_path_ambient_aud = os.path.join(
                        self.workspace, 'validation', f'{name}_{self.local_step:04d}_aud.png')
                    save_path_ambient_eye = os.path.join(
                        self.workspace, 'validation', f'{name}_{self.local_step:04d}_eye.png')
                    save_path_uncertainty = os.path.join(
                        self.workspace, 'validation', f'{name}_{self.local_step:04d}_uncertainty.png')
                    ...
                    if not self.opt.torso:
                        ...
                        cv2.imwrite(save_path_ambient_aud,
                                    (pred_ambient_aud * 255).astype(np.uint8))
                        cv2.imwrite(save_path_ambient_eye,
                                    (pred_ambient_eye * 255).astype(np.uint8))
                        cv2.imwrite(save_path_uncertainty,
