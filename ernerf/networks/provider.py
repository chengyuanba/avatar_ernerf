...
from ernerf.networks.utils import get_audio_features, get_bg_coords, get_rays
...
def smooth_camera_path(poses, kernel_size=5):
    ...
    for i in range(N):
        ...
        poses[i, :3, :3] = Rotation.from_matrix(
            rots[start:end]).mean().as_matrix()
...
class NeRFDataset_Test:
    def __init__(self, opt, device, downscale=1):
        ...
        if not self.opt.asr:
            ...
            print(
                f'[INFO] load {self.opt.aud} aud_features: {aud_features.shape}')
        ...
        for f in tqdm.tqdm(frames, desc=f'Loading data'):
            ...
            lms = np.loadtxt(os.path.join(
                self.root_path, 'ori_imgs', str(f['img_id']) + '.lms'))  # [68, 2]
            lh_xmin, lh_xmax = int(lms[31:36, 1].min()), int(
                lms[:, 1].max())  # actually lower half area
        ...
        if self.opt.bg_img == 'white':  # special
            ...
            bg_img = cv2.imread(
                self.opt.bg_img, cv2.IMREAD_UNCHANGED)  # [H, W, 3]
        ...
        if self.opt.smooth_path:
            self.poses = smooth_camera_path(
                self.poses, self.opt.smooth_path_window)
        ...
        if self.opt.exp_eye:
            ...
            print(
                f'[INFO] eye_area: {self.eye_area.min()} - {self.eye_area.max()}')
            ...
            self.eye_area = torch.from_numpy(
                self.eye_area).view(-1, 1)  # [N, 1]
        ...
        self.bg_coords = get_bg_coords(
            self.H, self.W, self.device)  # [1, H*W, 2] in [-1, 1]
    ...
    def collate(self, index):
        ...
        if self.auds is not None:
            auds = get_audio_features(
                self.auds, self.opt.att, index[0]).to(self.device)
            results['auds'] = auds

        # head pose and bg image may mirror
        index[0] = self.mirror_index(index[0])

        poses = self.poses[index].to(self.device)  # [B, 4, 4]

        rays = get_rays(poses, self.intrinsics, self.H, self.W,
                        self.num_rays, self.opt.patch_size)
        ...
        if self.opt.torso:
            bg_torso_img = self.torso_img[index[0]]
            if self.opt.preload == 0:
                bg_torso_img = cv2.imdecode(
                    np.fromfile(bg_torso_img, dtype='uint8'),
                    cv2.IMREAD_UNCHANGED)  # [H, W, 4]
                bg_torso_img = cv2.cvtColor(bg_torso_img, cv2.COLOR_BGRA2RGBA)
                bg_torso_img = bg_torso_img.astype('float32') / 255.
                bg_torso_img = torch.from_numpy(bg_torso_img).unsqueeze(0)
            bg_torso_img = bg_torso_img[..., :3] * bg_torso_img[..., 3:] + \
                self.bg_img * (1 - bg_torso_img[..., 3:])
    ...
    def dataloader(self):
        ...
        loader = DataLoader(list(range(size)), batch_size=1,
                            collate_fn=self.collate, shuffle=False, num_workers=0)
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1):
        ...
        if not self.opt.asr:
            ...
            if self.opt.aud == '':
                aud_feat_path = os.path.join(self.root_path, "audio_feat.npy")
                aud_features = np.load(aud_feat_path)
                print(f'[INFO] load audio features from: {aud_feat_path}')
            ...
            print(f'[INFO] audio features shape: {aud_features.shape}')
        ...
        for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
            ...
            f_path = os.path.join(self.root_path, 'gt_imgs',
                                  str(f['img_id']) + '.jpg')
            ...
            if self.preload > 0:
                torso_img = cv2.imread(
                    torso_img_path, cv2.IMREAD_UNCHANGED)  # [H, W, 4]
        ...
        if self.preload > 0:
            ...
            self.torso_img = torch.from_numpy(
                np.stack(self.torso_img, axis=0))  # [N, H, W, C]
        else:
    ...
    def collate(self, index):
        ...
        if self.training and self.opt.finetune_lips:
            ...
            results['rect'] = rect
            rays = get_rays(poses, self.intrinsics,
                            self.H, self.W, -1, rect=rect)
        else:
            rays = get_rays(poses, self.intrinsics, self.H,
                            self.W, self.num_rays, self.opt.patch_size)
        ...
        if self.training:
            ...
            face_mask = (rays['j'] >= xmin) & (rays['j'] < xmax) & (
                rays['i'] >= ymin) & (rays['i'] < ymax)  # [B, N]
            results['face_mask'] = face_mask

            xmin, xmax, ymin, ymax = self.lhalf_rect[index[0]]
            lhalf_mask = (rays['j'] >= xmin) & (rays['j'] < xmax) & (
                rays['i'] >= ymin) & (rays['i'] < ymax)  # [B, N]
        ...
        if self.opt.exp_eye:
            ...
            if self.training:
                ...
                eye_mask = (rays['j'] >= xmin) & (rays['j'] < xmax) & (
                    rays['i'] >= ymin) & (rays['i'] < ymax)  # [B, N]
        ...
        if self.preload == 0:  # on the fly loading
            bg_torso_img = cv2.imread(
                bg_torso_img[0], cv2.IMREAD_UNCHANGED)  # [H, W, 4]
            bg_torso_img = cv2.cvtColor(bg_torso_img, cv2.COLOR_BGRA2RGBA)
            bg_torso_img = bg_torso_img.astype(np.float32) / 255  # [H, W, 3/4]
            bg_torso_img = torch.from_numpy(bg_torso_img).unsqueeze(0)
        bg_torso_img = bg_torso_img[..., :3] * bg_torso_img[...,
                                                            3:] + self.bg_img * (1 - bg_torso_img[..., 3:])
        bg_torso_img = bg_torso_img.view(B, -1, 3).to(self.device)
        ...
        if self.training:
            bg_img = torch.gather(bg_img, 1, torch.stack(
                3 * [rays['inds']], -1))  # [B, N, 3]

        results['bg_color'] = bg_img

        if self.opt.torso and self.training:
            bg_torso_img = torch.gather(bg_torso_img, 1, torch.stack(
                3 * [rays['inds']], -1))  # [B, N, 3]
        ...
        if self.training:
            bg_coords = torch.gather(self.bg_coords, 1, torch.stack(
                2 * [rays['inds']], -1))  # [1, N, 2]
        else:
            bg_coords = self.bg_coords  # [1, N, 2]
