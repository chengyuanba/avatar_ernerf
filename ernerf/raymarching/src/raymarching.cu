    ...
    float *__restrict__ grid_dilation)
{
    ...
    if (x + 1 < H)
        res = fmaxf(res, grid[c * H3 + __morton3D(x + 1, y, z)]);
    if (x > 0)
        res = fmaxf(res, grid[c * H3 + __morton3D(x - 1, y, z)]);
    if (y + 1 < H)
        res = fmaxf(res, grid[c * H3 + __morton3D(x, y + 1, z)]);
    if (y > 0)
        res = fmaxf(res, grid[c * H3 + __morton3D(x, y - 1, z)]);
    if (z + 1 < H)
        res = fmaxf(res, grid[c * H3 + __morton3D(x, y, z + 1)]);
    if (z > 0)
        res = fmaxf(res, grid[c * H3 + __morton3D(x, y, z - 1)]);
    ...
    scalar_t *grad_ambient)
{
    ...
    while (step < num_steps)
    {
        ...
        // write grad_sigmas
        grad_sigmas[0] = deltas[0] * (grad_image[0] * (T * rgbs[0] - (r_final - r)) +
