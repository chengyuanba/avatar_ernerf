...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, step_counter=None, mean_count=-1, perturb=False, align=-1, force_all_rays=False, dt_gamma=0, max_steps=1024):
        ...
        _backend.march_rays_train(rays_o, rays_d, density_bitfield, bound, dt_gamma, max_steps, N, C, H, M,
                                  nears, fars, xyzs, dirs, deltas, rays, step_counter, noises)  # m is the actually used points number
...
class _composite_rays_train(Function):
    ...
    def forward(ctx, sigmas, rgbs, ambient, deltas, rays, T_thresh=1e-4):
        ...
        _backend.composite_rays_train_forward(
            sigmas, rgbs, ambient, deltas, rays, M, N, T_thresh, weights_sum, ambient_sum, depth, image)

        ctx.save_for_backward(sigmas, rgbs, ambient, deltas,
                              rays, weights_sum, ambient_sum, depth, image)
    ...
    def backward(ctx, grad_weights_sum, grad_ambient_sum, grad_depth, grad_image):
        ...
        _backend.composite_rays_train_backward(grad_weights_sum, grad_ambient_sum, grad_image, sigmas, rgbs, ambient,
                                               deltas, rays, weights_sum, ambient_sum, image, M, N, T_thresh, grad_sigmas, grad_rgbs, grad_ambient)
...
class _march_rays(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, bound, density_bitfield, C, H, near, far, align=-1, perturb=False, dt_gamma=0, max_steps=1024):
        ...
        if perturb:
            ...
            noises = torch.rand(n_alive, dtype=rays_o.dtype,
                                device=rays_o.device)
        else:
            noises = torch.zeros(
                n_alive, dtype=rays_o.dtype, device=rays_o.device)

        _backend.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, bound,
                            dt_gamma, max_steps, C, H, density_bitfield, near, far, xyzs, dirs, deltas, noises)
...
class _composite_rays_ambient(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambients, weights_sum, depth, image, ambient_sum, T_thresh=1e-2):
        _backend.composite_rays_ambient(n_alive, n_step, T_thresh, rays_alive, rays_t,
                                        sigmas, rgbs, deltas, ambients, weights_sum, depth, image, ambient_sum)
        return tuple()
...
class _composite_rays_train_sigma(Function):
    ...
    def forward(ctx, sigmas, rgbs, ambient, deltas, rays, T_thresh=1e-4):
        ...
        _backend.composite_rays_train_sigma_forward(
            sigmas, rgbs, ambient, deltas, rays, M, N, T_thresh, weights_sum, ambient_sum, depth, image)
    ...
    def backward(ctx, grad_weights_sum, grad_ambient_sum, grad_depth, grad_image):
        ...
        _backend.composite_rays_train_sigma_backward(grad_weights_sum, grad_ambient_sum, grad_image, sigmas, rgbs,
                                                     ambient, deltas, rays, weights_sum, ambient_sum, image, M, N, T_thresh, grad_sigmas, grad_rgbs, grad_ambient)
...
class _composite_rays_ambient_sigma(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambients, weights_sum, depth, image, ambient_sum, T_thresh=1e-2):
        _backend.composite_rays_ambient_sigma(
            n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, rgbs, deltas, ambients, weights_sum, depth, image, ambient_sum)
        return tuple()
...
class _composite_rays_train_uncertainty(Function):
    ...
    def forward(ctx, sigmas, rgbs, ambient, uncertainty, deltas, rays, T_thresh=1e-4):
        ...
        weights_sum = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        ambient_sum = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        uncertainty_sum = torch.empty(
            N, dtype=sigmas.dtype, device=sigmas.device)
        depth = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        image = torch.empty(N, 3, dtype=sigmas.dtype, device=sigmas.device)

        _backend.composite_rays_train_uncertainty_forward(
            sigmas, rgbs, ambient, uncertainty, deltas, rays, M, N, T_thresh, weights_sum, ambient_sum, uncertainty_sum, depth, image)

        ctx.save_for_backward(sigmas, rgbs, ambient, uncertainty, deltas,
                              rays, weights_sum, ambient_sum, uncertainty_sum, depth, image)
        ctx.dims = [M, N, T_thresh]
    ...
    def backward(ctx, grad_weights_sum, grad_ambient_sum, grad_uncertainty_sum, grad_depth, grad_image):
        ...
        _backend.composite_rays_train_uncertainty_backward(grad_weights_sum, grad_ambient_sum, grad_uncertainty_sum, grad_image, sigmas, rgbs, ambient,
                                                           uncertainty, deltas, rays, weights_sum, ambient_sum, uncertainty_sum, image, M, N, T_thresh, grad_sigmas, grad_rgbs, grad_ambient, grad_uncertainty)
...
class _composite_rays_uncertainty(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambients, uncertainties, weights_sum, depth, image, ambient_sum, uncertainty_sum, T_thresh=1e-2):
        _backend.composite_rays_uncertainty(n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, rgbs,
                                            deltas, ambients, uncertainties, weights_sum, depth, image, ambient_sum, uncertainty_sum)
        return tuple()
...
class _composite_rays_train_triplane(Function):
    ...
    def forward(ctx, sigmas, rgbs, amb_aud, amb_eye, uncertainty, deltas, rays, T_thresh=1e-4):
        ...
        _backend.composite_rays_train_triplane_forward(sigmas, rgbs, amb_aud, amb_eye, uncertainty, deltas,
                                                       rays, M, N, T_thresh, weights_sum, amb_aud_sum, amb_eye_sum, uncertainty_sum, depth, image)

        ctx.save_for_backward(sigmas, rgbs, amb_aud, amb_eye, uncertainty, deltas,
                              rays, weights_sum, amb_aud_sum, amb_eye_sum, uncertainty_sum, depth, image)
        ctx.dims = [M, N, T_thresh]
    ...
    def backward(ctx, grad_weights_sum, grad_amb_aud_sum, grad_amb_eye_sum, grad_uncertainty_sum, grad_depth, grad_image):
        ...
        _backend.composite_rays_train_triplane_backward(grad_weights_sum, grad_amb_aud_sum, grad_amb_eye_sum, grad_uncertainty_sum, grad_image, sigmas, rgbs, amb_aud, amb_eye, uncertainty,
                                                        deltas, rays, weights_sum, amb_aud_sum, amb_eye_sum, uncertainty_sum, image, M, N, T_thresh, grad_sigmas, grad_rgbs, grad_amb_aud, grad_amb_eye, grad_uncertainty)
...
class _composite_rays_triplane(Function):
    ...
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, deltas, ambs_aud, ambs_eye, uncertainties, weights_sum, depth, image, amb_aud_sum, amb_eye_sum, uncertainty_sum, T_thresh=1e-2):
        _backend.composite_rays_triplane(n_alive, n_step, T_thresh, rays_alive, rays_t, sigmas, rgbs, deltas,
                                         ambs_aud, ambs_eye, uncertainties, weights_sum, depth, image, amb_aud_sum, amb_eye_sum, uncertainty_sum)
        return tuple()
