...
original_BFM = loadmat(
    os.path.join(pretrained_weights_dir, "3DMM", "01_MorphableModel.mat"))
sub_inds = np.load(
    os.path.join(pretrained_weights_dir, "3DMM", "topology_info.npy"),
    allow_pickle=True).item()["sub_inds"]
exp_info = np.load(
    os.path.join(pretrained_weights_dir, "3DMM", "exp_info.npy"),
...
np.save(
    # "3DMM/3DMM_info.npy",
    os.path.join(pretrained_weights_dir, "3DMM", "3DMM_info.npy"),
    {
        "mu_shape": mu_shape,
        "b_shape": b_shape,
