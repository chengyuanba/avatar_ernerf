...
def face_tracker_training(ori_img_dir: str,
                          num_frames: int,
                          ThreeDMM_dir: str,
                          img_h: int = 512,
                          img_w: int = 512
    ...
    """
    Params:
        ori_img_dir(str): ori image files dir;
        num_frames(int): number of frames;
        ThreeDMM_dir(str): 3DMM model files dir;
    ...
    lms, img_paths = load_dir(ori_img_dir, start_id, end_id)
    ...
    model_3dmm = Face_3DMM(ThreeDMM_dir, id_dim, exp_dim, tex_dim, point_num)
    ...
    renderer = Render_3DMM(ThreeDMM_dir, arg_focal, h, w, batch_size, device_render)

    sel_ids = np.arange(0, num_frames, int(
    ...
    optimizer_id_frame = torch.optim.Adam(
        [euler_angle, trans, exp_para, id_para], lr=0.01)
        ...
        for iter in range(50):
            ...
            if i > 0:
                ...
                loss_lap = cal_lap_loss(
                    [rott_geo_lap.reshape(
                        rott_geo_lap.shape[0], -1).permute(1, 0)],
                    [1.0]
                )
            else:
                geometry_lap = model_3dmm.forward_geo_sub(
    ...
    pt_weight_path = os.path.join(
        os.path.dirname(ori_img_dir), "track_params.pt")
    torch.save(
        {
        ...
        # os.path.join(os.path.dirname(ori_img_dir), "track_params.pt"),
        pt_weight_path
    )

    print(f"Track Params Weight saved at: {pt_weight_path}")
